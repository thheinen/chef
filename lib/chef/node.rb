...
class Chef
  class Node
    ...
    # a method to handle the renamed configuration from whitelist -> allowlist
    ...
    def allowlist_or_whitelist_config(level)
      if Chef::Config["#{level}_attribute_whitelist".to_sym]
        Chef.deprecated(:attribute_blacklist_configuration, "Attribute blacklist and whitelist configurations have been deprecated. Use the attribute allowlist and blocklist configs instead")
        Chef::Config["#{level}_attribute_whitelist".to_sym]
      else
        Chef::Config["#{level}_attribute_allowlist".to_sym]
      end
    end



    # a method to handle the renamed configuration from blacklist -> blocklist
    # and to throw a deprecation warning when the old configuration is set
    #
    # @param [String] level the attribute level
    def blocklist_or_blacklist_config(level)
      if Chef::Config["#{level}_attribute_blacklist".to_sym]
        ...
        Chef::Config["#{level}_attribute_blocklist".to_sym]
      end
    end

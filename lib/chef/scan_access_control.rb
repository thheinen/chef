...
class Chef
  ...
  class ScanAccessControl
    ...
    def lookup_uid
      unless (pwent = ::ChefIO::Etc.getpwuid(stat.uid)).nil?
        pwent.name
      else
        stat.uid
      end
    ...
    def lookup_gid
      unless (pwent = ::ChefIO::Etc.getgrgid(stat.gid)).nil?
        pwent.name
      else
        stat.gid
      end
    ...
    def stat
      @stat ||= if @new_resource.instance_of?(Chef::Resource::Link)
                  ::ChefIO::File.lstat(@new_resource.path)
                else
                  realpath = ::ChefIO::File.realpath(@new_resource.path)
                  ::ChefIO::File.stat(realpath)
                end
    end
  end
end

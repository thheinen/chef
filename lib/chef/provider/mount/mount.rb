    ...
    class Mount
      class Mount < Chef::Provider::Mount
        ...
        def mountable?
          ...
          if device_should_exist? && !::ChefIO::File.exists?(device_real)
            raise Chef::Exceptions::Mount, "Device #{@new_resource.device} does not exist"
          elsif @new_resource.mount_point != "none" && !::ChefIO::File.exists?(@new_resource.mount_point)
        ...
        def enabled?
          ...
          enabled = false
          unless ::ChefIO::File.exist?("/etc/fstab")
            logger.debug "/etc/fstab not found, treating mount as not-enabled"
            return
          end
          ::ChefIO::File.foreach("/etc/fstab") do |line|
            case line
            when /^[#\s]/
              next
        ...
        def enable_fs
          ...
          if @current_resource.enabled
            ...
            ::ChefIO::File.open("/etc/fstab", "a") do |fstab|
        ...
        def device_mount_regex
          if network_device?
          ...
          elsif ::ChefIO::File.symlink?(device_real)
            ...
            "(?:#{Regexp.escape(device_real)}|#{Regexp.escape(::ChefIO::File.expand_path(::ChefIO::File.readlink(device_real), ::ChefIO::File.dirname(device_real)))})"
          else
            Regexp.escape(device_real)
          end
        end
        ...
        def edit_fstab(remove: false)
          if @current_resource.enabled
            ...
            found = false
            ::ChefIO::File.readlines("/etc/fstab").reverse_each do |line|
            ...
            ::ChefIO::File.open("/etc/fstab", "w") do |fstab|

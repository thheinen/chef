    ...
    class Link < Chef::Provider
      ...
      def load_current_resource
        ...
        if file_class.symlink?(current_resource.target_file)
          ...
          if ::ChefIO::File.exists?(current_resource.target_file)
            if ::ChefIO::File.exists?(new_resource.to) &&
      ...
      action :create do
        ...
        if current_resource.to != canonicalize(new_resource.to) ||
          ...
          if current_resource.to
            ...
            if ChefUtils.windows? && ::ChefIO::File.directory?(new_resource.to) &&
                ::ChefIO::File.directory?(current_resource.target_file)
              converge_by("unlink existing windows symlink to dir at #{new_resource.target_file}") do
                ::ChefIO::Dir.unlink(new_resource.target_file)
              end
            else
              converge_by("unlink existing symlink to file at #{new_resource.target_file}") do
                ::ChefIO::File.unlink(new_resource.target_file)
              end
            end
          end
      ...
      action :delete do
        if current_resource.to # Exists
          if ChefUtils.windows? && ::ChefIO::File.directory?(current_resource.target_file)
            converge_by("delete link to dir at #{new_resource.target_file}") do
              ::ChefIO::Dir.delete(new_resource.target_file)
              logger.info("#{new_resource} deleted")
            end
          else
            converge_by("delete link to file at #{new_resource.target_file}") do
              ::ChefIO::File.delete(new_resource.target_file)
              logger.info("#{new_resource} deleted")
            end
          end
        end

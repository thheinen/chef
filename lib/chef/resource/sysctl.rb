    ...
    class Sysctl < Chef::Resource
      ...
      **Converting sysctl settings from shell scripts**:
      ...
      action :remove, description: "Remove the kernel parameter and update the `sysctl` settings." do
        ...
        if ChefIO::File.exist?("#{new_resource.conf_dir}/99-chef-#{new_resource.key.tr("/", ".")}.conf")
      ...
      def get_sysctld_value(key)
        raise unless ChefIO::File.exist?("/etc/sysctl.d/99-chef-#{key.tr("/", ".")}.conf")

        k, v = ChefIO::File.read("/etc/sysctl.d/99-chef-#{key.tr("/", ".")}.conf").match(/(.*) = (.*)/).captures
